...
class ReviewDataset(Dataset):
    def __init__(self, filepath, tokenizer):
        ...
        # drop middle of the pack ratings, only keep significantly positive/negative ratings
        self.data = self.data[(self.data.rating <= 2) | (self.data.rating >= 9)].reset_index(drop=True)
        self.data['sentiment'] = np.where(self.data.rating < 5, 0, 1)
        # self.data['sentiment'] = self.data.rating.where(self.data.rating < 5, 1)
        # self.data['sentiment'] = self.data.rating.where(self.data.rating > 5, 0)
    ...
    def __getitem__(self, idx):
        ...
        sentiment = torch.tensor(self.data.sentiment[idx]).float()

        return dict(input_ids=input_ids, attention_mask=attention_mask), sentiment
...
class RatingModel(nn.Module):
    def __init__(self, bert_hidden_size=768, hidden_size=256):
        ...
        self.linear1 = nn.Linear(bert_hidden_size, 1024)
...
if __name__ == '__main__':
    ...
    model = RatingModel(hidden_size=1_000).to(device)
